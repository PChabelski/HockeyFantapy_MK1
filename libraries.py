...
def online_data_parser(year, dates_of_interest, control_file):
    ...
    if control_file["Online Parser Status"]['overall_status']:
        ...
        year2 = year + 1
        urlYear = str(year) + str(year2)
        dateResetCounter = 0

        testUrl = f'https://www.hockey-reference.com/leagues/NHL_{year + 1}_games.html'
        gamesPage = urlopen(testUrl)
        ...
        for a in excelPop.find_all('a', href=True):
            #print(f"a['href'][0:11] -> {a['href'][0:11]}")
            if a['href'][0:11] == '/boxscores/':
                #print(f"a[href]11:-5 -> {a['href'][11:-5]}")
                if len(a['href'][11:-5]) > 11:
                    gameDay = a['href'][11:-5]
                    #print(f'Game day: {gameDay}')
                    listOfGames.append(gameDay)

        gamelog = []
        for game in listOfGames:
            date = datetime.strptime(game[0:8], '%Y%m%d').strftime('%Y-%m-%d')
            if date in dates_of_interest:
                gamelog.append([date, game])
        gameRegisterDf = pd.DataFrame(gamelog, columns=['Date', 'URLCODE'])
        #print(gameRegisterDf)
        for date_item in gameRegisterDf['Date'].unique():
            print(f'Online parser date item: {date_item}')
            hr_df = pd.DataFrame()
            nst_df = pd.DataFrame()
            yahoo_df = pd.DataFrame()
            gameRegisterFiltered = gameRegisterDf[gameRegisterDf['Date'] == date_item]
            try:
                week_number = schedule_df[schedule_df['date'] == date_item]['week'].values[0]
            except:
            ...
            if control_file["Online Parser Status"]['hr_parse']:
                #                 try:
                hr_df = hockeyReference_parser(gameRegisterFiltered, year)
            ...
            if control_file["Online Parser Status"]['nst_parse']:
                try:
                    nst_df = natStatTrick_parser(gameRegisterFiltered, urlYear)
